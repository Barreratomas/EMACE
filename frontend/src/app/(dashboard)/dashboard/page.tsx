...
// Simple no-op subscription for hydration-safe mount detection
const subscribe = () => () => {};
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Agentes Activos', value: '12', icon: Users, color: 'text-primary', bg: 'bg-primary/10', glow: 'shadow-primary/5', trend: '+2 NEW', trendColor: 'text-cyber-lime' },
          { label: 'Operaciones 24h', value: '1,429', icon: Activity, color: 'text-primary', bg: 'bg-primary/10', glow: 'shadow-primary/5', trend: '+14% INC', trendColor: 'text-cyber-lime' },
          { label: 'Latencia Media', value: '24ms', icon: Zap, color: 'text-safety-orange', bg: 'bg-safety-orange/10', glow: 'shadow-safety-orange/5', trend: '-2ms DEC', trendColor: 'text-cyber-lime' },
          { label: 'Integridad', value: '99.9%', icon: Shield, color: 'text-cyber-lime', bg: 'bg-cyber-lime/10', glow: 'shadow-cyber-lime/5', trend: 'STABLE', trendColor: 'text-slate-500' },
        ].map((stat, i) => (
          <div key={i} className={cn("panel-industrial p-5 group relative overflow-hidden hover:border-primary/30 transition-all duration-300 rounded-none border-white/5", stat.glow)}>
            <div className="absolute inset-0 bg-linear-to-b from-transparent via-primary/5 to-transparent -translate-y-full group-hover:animate-scan pointer-events-none" />
            
            <div className="flex justify-between items-start mb-6 relative z-10">
              <div className={cn("p-2.5 border border-white/10 bg-black/40", stat.color)}>
                <stat.icon size={18} />
              </div>
              <span className={cn("text-[9px] font-bold uppercase tracking-[0.2em] px-2 py-0.5 border border-white/5 bg-black/20 terminal-text", stat.trendColor)}>{stat.trend}</span>
            </div>
            
            <div className="relative z-10">
              <div className="text-2xl font-black tracking-tighter mb-1 terminal-text">{stat.value}</div>
              <div className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.25em] terminal-text">{stat.label}</div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {[
                { name: 'EMACE_INVENTARIO', status: 'Online', load: 45, activity: 'SYNC_SK-42', type: 'System' },
                { name: 'EMACE_LOGISTICA', status: 'Online', load: 12, activity: 'OPT_ROUTES_B', type: 'Core' },
                { name: 'EMACE_VENTAS', status: 'Busy', load: 88, activity: 'PROC_BULK_#892', type: 'Client' },
                { name: 'EMACE_ANALITICA', status: 'Offline', load: 0, activity: 'MAINTENANCE_MODE', type: 'Service' },
              ].map((agent, i) => (
                <div key={i} className="panel-industrial p-4 flex items-center gap-5 group hover:bg-white/5 transition-all rounded-none border-white/5 relative overflow-hidden">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[9px] font-bold text-slate-500 terminal-text opacity-60 uppercase tracking-widest">[{agent.activity}]</div>
                  </div>
                  ...
                  <div className="text-right min-w-24">
                    ...
                    <div className="text-[8px] font-bold text-slate-600 mt-1 uppercase terminal-text tracking-widest">UPTIME: 14D</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="panel-industrial p-6 h-72 relative flex flex-col justify-between rounded-none border-white/5 overflow-hidden group">
              ...
              <div className="relative flex-1 mt-4">
                <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 500 200">
                  ...
                  {/* Area fill */}
                  <path 
                    d="M 0 150 L 50 120 L 100 130 L 150 80 L 200 100 L 250 40 L 300 60 L 350 20 L 400 50 L 450 30 L 500 40 V 200 H 0 Z" 
                    fill="url(#gradient-primary)" 
                    className="transition-all duration-1000"
                  />
                  {/* Line */}
                  <path 
                    d="M 0 150 L 50 120 L 100 130 L 150 80 L 200 100 L 250 40 L 300 60 L 350 20 L 400 50 L 450 30 L 500 40" 
                    fill="none" 
                    stroke="var(--primary)" 
                    strokeWidth="2"
                    strokeLinecap="round"
                  ...
                  {[0, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500].map((x, i) => {
                    const points = [150, 120, 130, 80, 100, 40, 60, 20, 50, 30, 40];
                    return (
                      <circle 
                        key={i} 
                        cx={x} 
              ...
              <div className="flex justify-between items-center mt-4 border-t border-white/5 pt-4">
                ...
                <div className="text-[8px] font-bold text-primary terminal-text animate-pulse">LIVE_STREAM_ACTIVE</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="panel-industrial p-0 overflow-hidden rounded-none border-white/5 group/feed relative">
            ...
            <div className="max-h-620px overflow-y-auto custom-scrollbar">
              <div className="divide-y divide-white/5">
                {[
                  { type: 'info', msg: 'STOCK_UPDATED: PRODUCT_SK-22', time: '02M_AGO', icon: MessageSquare, color: 'text-primary', bg: 'bg-primary/10' },
                  { type: 'warning', msg: 'HIGH_LOAD: AGENT_SALES', time: '12M_AGO', icon: AlertTriangle, color: 'text-safety-orange', bg: 'bg-safety-orange/10' },
                  { type: 'success', msg: 'SYSTEM_BACKUP_COMPLETE', time: '01H_AGO', icon: CheckCircle2, color: 'text-cyber-lime', bg: 'bg-cyber-lime/10' },
                  { type: 'error', msg: 'CONN_FAILURE: NODE_SECTOR_7', time: '03H_AGO', icon: AlertTriangle, color: 'text-rose-500', bg: 'bg-rose-500/10' },
                  { type: 'info', msg: 'NEW_USER_REG: ID_9281', time: '05H_AGO', icon: Users, color: 'text-primary', bg: 'bg-primary/10' },
                  { type: 'info', msg: 'CACHE_REVALIDATION_FIN', time: '06H_AGO', icon: Activity, color: 'text-slate-500', bg: 'bg-slate-500/10' },
                ].map((log, i) => (
                  <div key={i} className="p-4 hover:bg-white/5 transition-all flex gap-4 group relative overflow-hidden">
                    ...
                    <div className={cn("mt-1 p-2 border border-white/10 bg-black/40 transition-all duration-300 group-hover:border-primary/30", log.color)}>
                      <log.icon size={14} />
                    </div>
