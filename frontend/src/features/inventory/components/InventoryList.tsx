  ...
  return (
    <div>
      <div className="space-y-8 animate-in fade-in duration-500">
        <SectionHeader
          className="md:flex-row md:justify-between md:items-center pb-8 border-b border-border-ui/50"
          rightClassName="md:w-auto md:justify-end"
          left={
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                Gestione y supervise los activos críticos de la infraestructura.
              </p>
            </div>
          }
          right={
                  ...
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <div
                      className={`w-10 h-5 rounded-full p-1 transition-colors ${
                        lowOnly ? 'bg-primary' : 'bg-slate-200 dark:bg-slate-800'
                      }`}
                    >
                      <div
                        className={`w-3 h-3 bg-white rounded-full transition-transform ${
                          lowOnly ? 'translate-x-5' : 'translate-x-0'
                        }`}
                      />
                    </div>
                    <input
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-border-ui/30">
                    {filtered.map((p) => {
                      const isLow =
                        p.type === 'physical' &&
                        (p.stock || 0) <= (p.min_stock_threshold || 5);
                      return (
                        <tr
                          key={p.id}
                          className={`group transition-colors hover:bg-primary/2 cursor-default ${
                        ...
                        >
                          ...
                          <td className="p-5">
                            <div className="flex flex-col gap-1">
                              ...
                              <span
                                className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md w-fit uppercase ${
                                  p.type === 'physical'
