...
from app.domain.models import VendorAccessState, User, VendorMtprotoSession
...
async def lifespan(app: FastAPI):
    """Eventos de ciclo de vida de la aplicación (Lifespan)."""
    # --- STARTUP ---
    ...
    if settings.TELEGRAM_MTPROTO_ENABLED and not settings.TELEGRAM_MTPROTO_KILL_SWITCH:
        from app.infrastructure.adapters.botfather_orchestrator import botfather_orchestrator, BOTFATHER_ID
        ...
        mtproto_task = asyncio.create_task(mtproto_loop())
    ...
    if mtproto_task:
        ...
        try:
            ...
            logger.error(f"Error cancelling mtproto_task: {e}")
            
    try:
        await mtproto_manager.shutdown()
    except Exception as e:
        logger.error(f"Error shutting down mtproto_manager: {e}")
